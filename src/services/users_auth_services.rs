...
pub async fn users_pw_auth_login(pool: &MySqlPool, request: LoginRequest) -> Result<LoginResponse, UserError> {
    ...
    let record = row.ok_or_else(|| UserError::Unauthorized)?;
...
pub async fn users_pw_auth_register(pool: &MySqlPool, request: RegisterRequest) -> Result<(), UserError> {
    //生成唯一 uuid
    ...
    if row.rows_affected() == 1 { Ok(()) } else { Err(UserError::UsernameAlreadyExists) }
}